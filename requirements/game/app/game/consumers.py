...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not await self.room.add_player(self):
            print(f"Room {self.room_name} is full", flush=True)
            self.room = None
            await self.accept()
            await self.close(code=3005)
            return
    ...
    async def disconnect(self, code):
        ...
        if len(self.room.players) == 2:
            winner  = self.room.players[0].user["alias"] if self.room.players[0] != self else self.room.players[1].user["alias"]
