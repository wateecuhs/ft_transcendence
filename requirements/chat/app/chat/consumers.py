...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        for contact in contacts:
            ...
            await self.channel_layer.group_send(
                f"user.{contact}",
                {"type": MessageType.Status.REQUEST, "data": {"author": self.user}},
            )
            await self.channel_layer.group_send(
                f"user.{contact}",
                {
                    "type": MessageType.Status.UPDATE,
                    "data": {"author": self.user, "status": cmod.User.Status.ONLINE},
                },
            )
    ...
    async def disconnect(self, close_code):
        if self.user is None:
            logger.info("[ANON] User is not authenticated.")
            return
        ...
        for contact in contacts:
            ...
            await self.channel_layer.group_send(
                ...
                {
                    "type": MessageType.Status.UPDATE,
                    "data": {"author": self.user, "status": cmod.User.Status.OFFLINE},
                },
            )
    ...
    async def _handle_chat_message(self, event: Dict[str, Any]):
        ...
        if message[0] == "/":
            match message.split(" ")[0]:
                ...
                case (
                    self.ADD_FRIEND_CMD
                    | self.REMOVE_FRIEND_CMD
                    | self.ACCEPT_FRIEND_CMD
    ...
    async def _handle_block_command(self, message: str):
        ...
        if len(splitted_message) != 2:
            await self.error(f"Invalid block command format. ({message})")
            return

        target = splitted_message[1]
        if target == self.user:
            logger.info("Handling block command: You can't block yourself.")
    ...
    async def _handle_friend_command(self, message: str):
        ...
        splitted_message = message.split(" ")
        if len(splitted_message) != 2:
            await self.error(f"Invalid friend command format. ({message})")
            return

        target = splitted_message[1]
        if target == self.user_id:
            await self.error("You can't add/remove yourself.")
            return
            ...
            match splitted_message[0]:
                case self.ADD_FRIEND_CMD:
                    ...
                    await self.channel_layer.group_send(
                        f"user.{target}",
                        {
                            "type": MessageType.Relationship.REQUEST,
                ...
                case self.REMOVE_FRIEND_CMD:
                    ...
                    await self.channel_layer.group_send(
                        f"user.{target}",
                        {
                            "type": MessageType.Relationship.REMOVE,
                            "data": {"author": self.user},
                        },
                    )
                    await self.channel_layer.group_send(
                ...
                case self.ACCEPT_FRIEND_CMD:
                    ...
                    await self.channel_layer.group_send(
                        f"user.{target}",
                        {
                            "type": MessageType.Relationship.ACCEPT,
                            "data": {"author": self.user},
                        },
                    )
                    await self.channel_layer.group_send(
                ...
                case _:
                    await self.error(f"Invalid friend command. ({message})")
        except Exception as e:
            await self.error(f"Error in friend command ({e})")
    ...
    async def _handle_private_message(self, message: str):
        ...
        if len(splitted_message) < 3:
            await self.error(f"Invalid private message format. ({message})")
            return
        ...
        if target == self.user_id:
            await self.error("You can't send a message to yourself.")
            return
        ...
        if (
            await database_sync_to_async(get_relationship_status)(self.user, target)
            == cmod.Relationship.Status.BLOCKED
            or await database_sync_to_async(get_relationship_status)(target, self.user)
            == cmod.Relationship.Status.BLOCKED
        ):
            await self.error(f"User {target} has blocked you.")
            return
        await self.channel_layer.group_send(
            f"user.{target}", {"type": MessageType.Chat.PRIVATE, "data": message_data}
        )
    ...
    async def status_update(self, event):
        logger.info(f"[{self.user}] Sending status update: {event['data']}")
        event["data"]["content"] = (
            f"{event['data']['author']} is now {event['data']['status']}."
        )
    ...
    async def chat_public(self, event):
        try:
            ...
            if (
                await database_sync_to_async(get_relationship_status)(self.user, author)
                == cmod.Relationship.Status.BLOCKED
                or await database_sync_to_async(get_relationship_status)(
                    author, self.user
                )
                == cmod.Relationship.Status.BLOCKED
            ):
